    ...
    <Modal {...props} aria-labelledby="contained-modal-title-vcenter">
            ...
            <FormControl variant="standard" style={{ margin: "19px" }}>
              <div style={{ height: "85px" }}>
                <div>
                  <TextField
                    label="Old Password"
                    type={oldHide === true ? "password" : "text"}
              ...
              <div style={{ height: "85px" }}>
                <div>
                  <TextField
                    type={newHide === true ? "password" : "text"}
                    label="New Password"
                    error={errorMessageNew.status}
                    color="primary"
                    onChange={onInputChangeNewPassword}
                  />
                  {newHide === true ? (
                    <AiFillEyeInvisible
                      onClick={() => setNewHide(!newHide)}
                      style={{
                        position: "absolute",
                        display: "flex",
                        right: "19px",
              ...
              <div style={{ height: "85px" }}>
                <div>
                  <TextField
                    ...
                    error={errorMessageConfirm.status}
                    color="primary"
                    onChange={onInputChangeConfirmNewPassword}
                  />
                  {newConfirmHide === true ? (
                    <AiFillEyeInvisible
                      onClick={() => setNewConfirmHide(!newConfirmHide)}
                      style={{
                        position: "absolute",
                        display: "flex",
                        right: "19px",
      ...
      >
        <ModalBtnContainer>
          ...
          {errorMessageOld.status === false &&
          errorMessageNew.status === false &&
          errorMessageConfirm.status === false ? (
            <Button
              className="modalBtn "
              onClick={() => {
                // props.onConfirm;
